...
在过去十年间陆陆续续收集了很多喜欢的字体，然而庞大的字体库很难在各种地方都管理好。之前尝试过“字由”这种APP，作为非专业的设计师，持续付费的意愿低，还有些自己想要的功能也没有。于是做了一个个人字体网站，功能集管理、预览、下载于一体。如果你在本地有目录组织的字体库的话，还能一键复刻这个网站。

一句话简介：一个基于 **Vue 3 + Vite** 的字体展示应用，用于以卡片形式浏览、预览和下载字体文件。支持多级分类树、字体重量子集化压缩、家族字重合并、实时预览和 CDN 原始文件下载。

![项目预览](image.png)

## 核心功能

- 🗂️ **多级分类树**：分类直接映射 `字体库/` 目录的文件夹层级（如 `中文字体/黑体/思源黑体`），支持面包屑导航与逐层筛选。
- 🧩 **字体卡片网格**：每款字体独立成卡，自动按家族合并多字重版本（如 Alibaba-PuHuiTi、HONORSansCN、思源黑体等）。
- 🔁 **多字重预览切换**：合并后的家族字体会显示字重按钮，点击可在同一卡片内切换对应字重的预览；批量获取仍通过“下载全部”按钮完成。
- ⚡ **子集化预览**：使用 `fontTools.subset` 将每款字体裁剪为仅含固定预览字符的 WOFF2 子集，大幅缩减加载体积。
- 🏗️ **静态数据构建**：字体列表在构建时通过 `src/fonts.json` 静态导入，保证首屏加载速度和确定性。
- 🚀 **本地目录一键建站**：基于用户已有的本地字体目录生成分类、预览子集和字体数据，快速构建可浏览、可下载的字体展示网站。
- ✍️ **自定义预览文字**：顶部输入框可实时修改所有卡片的预览内容。
- ⬇️ **一键下载原始字体**：下载按钮直接指向 CDN 直链，不占用仓库与部署包体积。
